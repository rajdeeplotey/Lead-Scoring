...
This system uses a **hybrid approach** combining rule-based scoring with optional machine learning predictions. It's important to understand how each component works:

### Rule-Based Scoring Engine (Primary Method)

The **rule-based scoring engine is the primary, transparent, production scoring method**. It evaluates businesses across 7 digital presence pillars using clearly defined rules:

- **Google Presence** (20 points): Based on Google rating (0-5) and review count tiers
- **Instagram Activity** (20 points): Based on account activity and follower count tiers
- **Website Status** (15 points): Based on website quality (None/Basic/Good)
- **WhatsApp Availability** (15 points): Binary (available/not available)
- **Menu/Catalog** (10 points): Binary (available/not available)
- **Branding Quality** (10 points): Based on branding assessment (Low/Medium/High)
- **Online Ordering** (10 points): Binary (available/not available)

This rule-based approach is **fully transparent** - every point can be traced back to a specific rule. This ensures sales teams can explain to clients exactly why they received a particular score.

### Machine Learning Layer (Optional/Experimental)

The ML layer is **optional and experimental**. When ML models are available, they provide additional predictions:

- **ml_lead_score**: ML-predicted lead score (regression)
- **ml_lead_status**: ML-predicted lead status (classification)

**Important Disclosure**: The current ML models are **trained on labels generated by the rule-based scoring engine itself**, NOT on real conversion/sales outcome data. This means:

- The ML models learn to replicate the rule-based logic
- They do not predict actual business conversion or sales outcomes
- Their metrics (accuracy, precision, etc.) reflect how well they learned the rules, not real-world predictive performance
- This is disclosed honestly as a **hybrid prototype**, not an independent AI prediction model

### Why This Hybrid Approach?

This hybrid design allows the system to:
1. **Provide transparent, explainable scoring** via the rule-based engine (production-ready)
2. **Experiment with ML patterns** that may eventually complement the rules
3. **Gracefully degrade** - the system works perfectly with rule-based scoring alone if ML models are unavailable
4. **Iterate toward data-driven improvements** when real conversion data becomes available
...
│   └── ludhiana_training_data_150.csv
...
2. Copy environment file and fill in your actual credentials:
```bash
cp .env.example .env
...
4. Install web backend dependencies:
```bash
